...
#include "karo/v1/move.pb.h"
#include "karo/v1/patrol.pb.h"
#include "karo/v1/events.pb.h"
...
{
  ::karo::v1::CreateMoveTaskRequest req;
  req.set_marker_id(marker_id);
  return encode_task_rpc(request_id, "move.create_task", req);
}
  ...
  uint64_t request_id, const std::string& task_id)
{
  ...
  return encode_task_rpc(request_id, "task_actions.cancel_task", req);
}
  ...
  uint64_t request_id, const std::string& task_id)
{
  ...
  req.set_task_id(task_id);
  return encode_task_rpc(request_id, "task_actions.pause_task", req);
}
  ...
  uint64_t request_id, const std::string& task_id)
{
  ...
  req.set_task_id(task_id);
  return encode_task_rpc(request_id, "task_actions.resume_task", req);
}
...
{
  ::karo::v1::CreateMoveTaskResponse resp;
  if (!resp.ParseFromArray(payload.data(), static_cast<int>(payload.size()))) {
    return false;
  }
  task_id_out = resp.task().task_id();
  return true;
}

namespace {
void flatten_task(const ::karo::v1::Task& t, TaskEvent& out)
{
...
{
  ::karo::v1::TaskEventEnvelope env;
  if (!env.ParseFromArray(payload.data(), static_cast<int>(payload.size()))) {
    return false;
  }
  // tasks.events 信封每帧含一种事件; SDK 只把含完整 Task 快照的
  // created / updated 透出为 TaskEvent. state_changed 与 updated 同时发出
  // (信息已被 updated 覆盖), deleted 当前设备端不产生 — 两者跳过.
  switch (env.kind_case()) {
    case ::karo::v1::TaskEventEnvelope::kCreated:
      flatten_task(env.created().task(), out);
      return true;
    case ::karo::v1::TaskEventEnvelope::kUpdated:
      flatten_task(env.updated().task(), out);
      return true;
    default:
      return false;
  }
