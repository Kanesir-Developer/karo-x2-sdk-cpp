...
std::unique_ptr<Subscription> RobotImpl::SubscribeGeneric(
  const std::string& topic, double desired_hz,
  Robot::TopicDataCallback on_data, StatusCallback on_status) {
  ...
  slot->frame_dispatcher = [on_data, weak_slot = std::weak_ptr<SubSlot>(slot)]
                          (const DecodedTopicPush& push) {
    auto s = weak_slot.lock();
    if (!s) return;
    TopicMessage m;
    m.topic = push.topic;
    m.timestamp_ms = push.timestamp_ms;
    m.seq = push.seq;
    m.payload = push.payload;
    try { on_data(m); } catch (...) {}
  };
  ...
  boost::asio::post(control_ioc_,
    [self = shared_from_this(), slot] {
      ...
      if (self->state_.load() != ConnectionState::Ready) {
        self->PostLog(LogLevel::Debug,
          "Subscribe queued (non-Ready): " + slot->topic);
        return;
      }
      self->SendSubscribe(slot, 0);
    });
...
std::unique_ptr<Subscription> RobotImpl::SubscribeRobotStatus(
  ...
  const std::string topic = "robot.state";
